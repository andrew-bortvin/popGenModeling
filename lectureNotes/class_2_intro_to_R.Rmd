...
We'll talk a lot more about this dataset and the biology it represents when we study predator-prey models, but for now let's use it to make some exploratory plots.
...
![](/Users/andrew/popGenModeling/01-intro_to_R/img/csv_head.png)
...
```{r}
read.table("~/popGenModeling/01-intro_to_R/hare_lynx.csv")
```
...
```{r}
read.table("~/popGenModeling/01-intro_to_R/hare_lynx.csv", sep = ",")
```
...
```{r}
read.table("~/popGenModeling/01-intro_to_R/hare_lynx.csv", sep = ",", header = TRUE)
```
...
```{r, eval = FALSE}
read.table("~/popGenModeling/01-intro_to_R/hare_lynx.csv", sep = ",", header = TRUE)
read.table("~/popGenModeling/01-intro_to_R/hare_lynx.csv", header = TRUE, sep = ",")
```
